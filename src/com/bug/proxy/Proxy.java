...
public class Proxy
{
    ...
    public Object create() throws Throwable
    {
        ...
        String ClassName=SuperClass.getName().replace(".", "/");
        String source = ClassName.replaceAll(".+?/(?=[^/]+$)|\\$.+$", "");
        //path = new File("/mnt/sdcard/apk/tmp");
        //父类
        TypeId<?> superClass=TypeId.get(SuperClass);
        //继承类
        TypeId <?> newClass = TypeId.get(superClass.getName().substring(0, superClass.getName().length() - 1) + "$Proxy;");
        //所有接口
        ...
        FieldId<?, ProxyCallback> callback_field=newClass.getField(callback, "callback");
        ...
        code.returnVoid();
        //增加设置回调方法
        MethodId<?, Void> setCallback=newClass.getMethod(TypeId.VOID, "setCallback", new TypeId<?>[]{callback});
        ...
        for (Method method:methods)
        {
            ...
            //方法参数类型
            TypeId<?>[] parameterTypes=new TypeId<?>[0];
            ...
            //调用原方法
            MethodId<?, ?> invokeSuper = newClass.getMethod(returnType, name + "_Super", parameterTypes);
            ...
            code.invokeSuper(TypeId.get(method.getDeclaringClass()).getMethod(returnType, name, parameterTypes), (Local)result, code.getThis(newClass), args);
            ...
            Local<Object> tmpObj=code.newLocal(TypeId.OBJECT);
            ...
            for (int i=0;i < parameterTypes.length;i++)
            {
                ...
                code.loadConstant(tmpObj, parameterTypes[i]);
            ...
            for (int i=0;i < parameterTypes.length;i++)
            {
                if (_parameterTypes[i].isPrimitive())
                {
                    ...
                    if (type == int.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Integer.class).getConstructor(TypeId.INT), code.getParameter(i, parameterTypes[i]));
                    }
                    else if (type == byte.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Byte.class).getConstructor(TypeId.BYTE), code.getParameter(i, parameterTypes[i]));
                    }
                    else if (type == short.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Short.class).getConstructor(TypeId.SHORT), code.getParameter(i, parameterTypes[i]));
                    }
                    else if (type == long.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Long.class).getConstructor(TypeId.LONG), code.getParameter(i, parameterTypes[i]));
                    }
                    else if (type == float.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Float.class).getConstructor(TypeId.FLOAT), code.getParameter(i, parameterTypes[i]));
                    }
                    else if (type == char.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Character.class).getConstructor(TypeId.CHAR), code.getParameter(i, parameterTypes[i]));
                    }
                    else if (type == double.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Double.class).getConstructor(TypeId.DOUBLE), code.getParameter(i, parameterTypes[i]));
                    }
                    else if (type == boolean.class)
                    {
                        code.newInstance(tmpObj, TypeId.get(Boolean.class).getConstructor(TypeId.BOOLEAN), code.getParameter(i, parameterTypes[i]));
                    }
                }
                else
                {
                    //移动第i个参数到缓存寄存器
                    code.move(tmpObj, code.getParameter(i, parameterTypes[i]));
                }
                //初始化数组下标
                code.loadConstant(index, i);
                //强制转换缓存对像为Object
            ...
            //通过类对象获取原方法对象
            MethodId<Class, Method> getDeclaredMethod = TypeId.get(Class.class).getMethod(TypeId.get(Method.class), "getDeclaredMethod", new TypeId[]{TypeId.get(String.class), TypeId.get(Class[].class)});
            //执行方法将返回值存入缓存寄存器
            code.invokeVirtual(getDeclaredMethod, tmpObj, mClass, new Local[]{methodName,allClass});
            //回调方法
            MethodId<ProxyCallback, Object> call=callback.getMethod(TypeId.OBJECT, "call", new TypeId[]{TypeId.OBJECT,TypeId.get(Method.class),TypeId.get(Object[].class)});
            //调用接口方法并存入返回值，同上
            code.invokeInterface(call, tmpObj, callbackObj, new Local[]{code.getThis(newClass),tmpObj,allArgs});
            //通过返回类型做不同处理
            ...
            if (type != void.class)
            {
                ...
                if (type == int.class)
                {
                    ...
                    code.invokeVirtual(get.getMethod(TypeId.INT, "intValue"), result, tmpObj);
                }
                else if (type == byte.class)
                {
                    TypeId<?> get = TypeId.get(Byte.class);
                    code.castType(tmpObj, get);
                    code.invokeVirtual(get.getMethod(TypeId.BYTE, "byteValue"), result, tmpObj);
                }
                else if (type == short.class)
                {
                    TypeId<?> get = TypeId.get(Short.class);
                    code.castType(tmpObj, get);
                    code.invokeVirtual(get.getMethod(TypeId.SHORT, "shortValue"), result, tmpObj);
                }
                else if (type == long.class)
                {
                    TypeId<?> get = TypeId.get(Long.class);
                    code.castType(tmpObj, get);
                    code.invokeVirtual(get.getMethod(TypeId.LONG, "longValue"), result, tmpObj);
                }
                else if (type == float.class)
                {
                    TypeId<?> get = TypeId.get(Float.class);
                    code.castType(tmpObj, get);
                    code.invokeVirtual(get.getMethod(TypeId.FLOAT, "floatValue"), result, tmpObj);
                }
                else if (type == char.class)
                {
                    TypeId<?> get = TypeId.get(Character.class);
                    code.castType(tmpObj, get);
                    code.invokeVirtual(get.getMethod(TypeId.CHAR, "charValue"), result, tmpObj);
                }
                else if (type == double.class)
                {
                    TypeId<?> get = TypeId.get(Double.class);
                    code.castType(tmpObj, get);
                    code.invokeVirtual(get.getMethod(TypeId.DOUBLE, "doubleValue"), result, tmpObj);
                }
                else if (type == boolean.class)
                {
                    TypeId<?> get = TypeId.get(Boolean.class);
                    code.castType(tmpObj, get);
                    code.invokeVirtual(get.getMethod(TypeId.BOOLEAN, "booleanValue"), result, tmpObj);
                }
                else
                {
                    //不是基本类型，强制转换为返回值的类型
            ...
            code.mark(lable);
            //未设置回调直接调用原方法
            code.invokeVirtual(invokeSuper, result, code.getThis(newClass), args);
            //跳转lable2
            code.jump(lable2);
        ...
        output.write(dexMaker.generate());
        output.close();
        ...
        deleteFile(path);
        //加载代理类
        Class<?> clazz=loader.loadClass(ClassName + "$Proxy");
        //获取之前定义好的无参数构造器
